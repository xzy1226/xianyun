...
export default {
  ...
  methods: {
    ...
    handleScreening(){
      ...
      // 筛选机场
      if(this.airport){
        arr=arr.filter(el=>el.org_airport_name==this.airport)
      }
      // 筛选出发时间
      if (this.flightTimes) {
        const {from,to}=this.flightTimes;
        arr=arr.filter(el=>{
          const start=+el.dep_time.split(':')[0]
          return start>=from && start<to;
        })
      }
      // 筛选公司
      if(this.company){
        arr=arr.filter(el=>el.airline_name==this.company)
      }
      // 筛选机型
      if(this.airSize){
        arr=arr.filter(el=>el.plane_size==this.airSize)
      }
